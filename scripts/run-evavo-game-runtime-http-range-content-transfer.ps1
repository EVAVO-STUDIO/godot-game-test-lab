...
if ([string]::IsNullOrWhiteSpace($ArtifactRoot)) {
    $ArtifactRoot = Join-Path $TestLabRoot `
        "artifacts\evavo-game-runtime-http-range-content-transfer"
}
...
if ($LASTEXITCODE -ne 0) {
    throw "EVAVO Game Runtime HTTP range Test Lab suite failed."
}

Write-Host "EVAVO Game Runtime HTTP range Test Lab suite passed."
