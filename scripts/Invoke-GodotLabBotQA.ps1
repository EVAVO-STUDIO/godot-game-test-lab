...
if ([System.Environment]::OSVersion.Platform -ne [System.PlatformID]::Win32NT) {
    throw "Native bot QA must run on Windows."
}
...
if (-not $AllowNonInteractive) {
    if ($currentSession -eq 0 -or $explorerSessions -notcontains $currentSession) {
        throw (
            "Bot QA requires Explorer in the worker's nonzero Windows session. " +
            "Launch the approved worker interactively, not as a Session 0 service."
        )
    }
}
else {
    Write-Warning (
        ...
        "native desktop or GPU journey evidence."
    )
}
...
Write-Host "[godot-lab] Running exact-SHA deterministic bot QA."
& $PythonExecutable @arguments
if ($LASTEXITCODE -ne 0) {
    throw "Godot bot QA failed with exit code $LASTEXITCODE."
}
Write-Host "[godot-lab] Deterministic bot QA passed."
