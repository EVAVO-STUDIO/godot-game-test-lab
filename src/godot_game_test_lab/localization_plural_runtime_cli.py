...
from .localization_plural import load_plural_testlab_request
from .localization_plural_runtime import run_plural_localization_runtime_validation
...
def _resolve_engine(args: argparse.Namespace, project_root: Path) -> Path | None:
    ...
    if args.no_auto_provision_engine:
        return None
    _, installation = ensure_project_engine(
        project_root,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m godot_game_test_lab.localization_plural_runtime_cli",
        description=(
            "Canonical guarded validator for an exact Localization Manager Godot plural artifact."
        ),
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        report = run_plural_localization_runtime_validation(
            project_root,
            request,
