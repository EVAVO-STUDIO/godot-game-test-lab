...
def parse_user_arguments(value: str) -> list[str]:
    ...
    for index, item in enumerate(parsed):
        ...
        if (
            ...
            raise JourneyError(
                f"visual user argument {index} must be a bounded --prefixed value."
            )
        result.append(item)
    return result
...
def safe_scene(value: str) -> str:
    ...
    if not scene.startswith("res://") or "\\" in scene or "\n" in scene or "\r" in scene:
        raise JourneyError("visual scene must be an empty value or a canonical res:// path.")
    parts = scene[6:].split("/")
...
def _write_command_logs(result: Any, artifacts: Path, name: str) -> list[str]:
    logs = artifacts / "logs"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if rendering_method not in ALLOWED_RENDERING_METHODS:
        raise JourneyError(
            "rendering method must be gl_compatibility, mobile, or forward_plus."
        )
    ...
    if base.status == "passed" and args.visual_frames > 0:
        ...
        if not xvfb:
            ...
            findings.append("governed-rendered-journey: xvfb-run is unavailable")
        else:
            command = [
                "/usr/bin/env",
            ...
            result = run_command(command, project_root, max(1, args.timeout))
            evidence = _write_command_logs(result, artifacts, "governed-rendered-journey")
            ...
            journey_status = "passed" if not journey_findings else "failed"
            checks.append(
                {
                    "id": "governed-rendered-journey",
            ...
            findings.extend(
                f"governed-rendered-journey: {item}" for item in journey_findings
            )
            ...
            if movie.is_file() and movie.stat().st_size > 0:
                ...
                if not ffprobe:
                    ...
                    findings.append("visual-probe: ffprobe is unavailable")
                else:
                    probe = run_command(
                        [
                            ffprobe,
                    ...
                    probe_findings = _command_findings(probe)
                    probe_path = visual / "ffprobe.json"
                    ...
                    checks.append(
                        {
                            ...
                            "evidence": _write_command_logs(
                                probe, artifacts, "profiled-visual-probe"
                            )
                            + (
                                [probe_path.relative_to(artifacts).as_posix()]
                                if probe_path.is_file()
                                else []
                            ),
                        }
                    )
                    findings.extend(f"visual-probe: {item}" for item in probe_findings)
                ...
                if not ffmpeg:
                    ...
                    findings.append("visual-screenshots: ffmpeg is unavailable")
                else:
                    ...
                    checks.append(
                        {
                            ...
                            "evidence": _write_command_logs(
                                extract, artifacts, "profiled-visual-screenshots"
                            )
                            + [path.relative_to(artifacts).as_posix() for path in frames],
                        }
                    )
                    findings.extend(
                        f"visual-screenshots: {item}" for item in extract_findings
                    )
                    ...
                    checks.append(
                        {
                            ...
                            "evidence": _write_command_logs(
                                sheet, artifacts, "profiled-visual-contact-sheet"
                            )
                            + (
                                [contact.relative_to(artifacts).as_posix()]
                                if contact.is_file()
                                else []
                            ),
                        }
                    ...
                    findings.extend(
                        f"visual-contact-sheet: {item}" for item in sheet_findings
                    )
